...
from library_api.types import BookCabinetId, BookId, CabinetId, UserId, OfficeId
...
class BookCabinet(Base):
    ...
    id: Mapped[BookCabinetId] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    ...
    update_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
